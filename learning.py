...
def back_progagation(net, x, t):    # non sto considerando bias
    ...
    for i in range(net.n_layers-1,-1,-1):
        if i == net.n_layers-1 :
            delta[i] = net.act_fun_deriv_per_layer[i](layer_input[i]) * net.error_fun_deriv(layer_output[i])
        else :
            delta[i] = net.act_fun_deriv_per_layer[i](layer_input[i]) * np.dot(np.transpose(net.weights[i+1]),delta[i+1])
